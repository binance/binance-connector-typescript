...
describe('Cancel Replace Order', () => {
    ...
    const callbacks = {
        open: (client: WebsocketAPI) => client.cancelReplaceOrder('BNBUSDT', CancelReplaceMode.ALLOW_FAILURE, 3285225, Side.BUY, OrderType.LIMIT, {
    ...
    it('should cancel an existing order and immediately place a new order instead of the canceled one', async () => {
        resultTemplate.data = mockCancelReplaceOrder;

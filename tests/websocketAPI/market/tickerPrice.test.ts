...
describe('Ticker Price', () => {
    let responseMessage: WebSocket.Data = '';
    let server: http.Server;
    const callbacks = {
        open: (client: WebsocketAPI) => client.tickerPrice({ symbol: 'BTCUSDT' }),
    ...
    it('should return the latest market price for a symbol', async () => {
        resultTemplate.data = mockTickerPrice;
